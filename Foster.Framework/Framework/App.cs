...
public static class App
{
	...
	public static bool DidFixedUpdate { get; private set; } = false;
	...
	public static void Run(string applicationName, int width, int height, bool fullscreen = false, Renderers renderer = Renderers.None)
	{
		...
		Time.FixedAccumulator = TimeSpan.Zero;
		timer.Restart();
	...
	private static void Tick()
	{
		...
		Time.FixedAccumulator += deltaTime;

		// Do as many fixed updates as we can
		while (Time.FixedAccumulator >= Time.FixedStepTarget)
		{
			DidFixedUpdate = true;
			Time.FixedAccumulator -= Time.FixedStepTarget;

			for (int i = 0; i < modules.Count; i++)
				modules[i].FixedUpdate();
		}

 		Time.FixedAlpha = (float)(Time.FixedAccumulator / Time.FixedStepTarget);
		...
		var timeToWait = Time.FramerateTarget - (timer.Elapsed - currentTime);

		if (timeToWait.TotalMilliseconds > 0)
		{
			var timeToWaitLazy = timeToWait - TimeSpan.FromMilliseconds(0.25f);
			Thread.Sleep(timeToWaitLazy);

			while ((Time.FramerateTarget - (timer.Elapsed - currentTime)).TotalMilliseconds > 0) { };
		}
	}
