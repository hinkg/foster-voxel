...
public class Texture : IResource
{
	...
	/// <summary>
	/// Copies given buffer to a region inside the texture
	/// </summary>
	public unsafe void SetSubData<T>(ReadOnlySpan<T> data, int offsetX, int offsetY, int width, int height) where T : struct
	{
		if (IsDisposed)
			throw new Exception("Resource is Disposed");

		if (data.Length != width * height)
			throw new Exception("Data Buffer size does not match Width*Height");

		if (offsetX < 0 || offsetY < 0 || offsetX + width > Width || offsetY + height > Height)
			throw new Exception("Data rect is outside texture bounds");

		fixed (byte* ptr = MemoryMarshal.AsBytes(data))
		{
			Platform.FosterTextureSetSubData(resource, ptr, offsetX, offsetY, width, height);
		}
	}
