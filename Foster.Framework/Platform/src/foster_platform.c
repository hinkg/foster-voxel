...
void FosterSetRelativeMouseMode(int mode)
{
	FOSTER_ASSERT_RUNNING(FosterGetFocused);
	SDL_SetRelativeMouseMode(mode);
}

void FosterSetMousePosition(int x, int y) 
{
	FOSTER_ASSERT_RUNNING(FosterSetMousePosition);
	SDL_WarpMouseInWindow(fstate.window, x, y);
}
...
void FosterTextureSetSubData(FosterTexture* texture, void* data, int offsetX, int offsetY, int width, int height)
{
	FOSTER_ASSERT_RUNNING(FosterTextureSetSubData);
	fstate.device.textureSetSubData(texture, data, offsetX, offsetY, width, height);
}
