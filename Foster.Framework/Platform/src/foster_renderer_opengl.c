...
#define GL_FUNCTIONS \
	...
	GL_FUNC(TexSubImage2D, void, GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void* data) \
...
typedef struct FosterTexture_OpenGL
{
	GLuint id;
	int width;
	int height;
...
#undef GL_FUNC
	...
	GLuint stateArrayBuffer;
	GLuint stateElementBuffer;
...
FosterUniformType FosterUniformTypeFromGL(GLenum value)
{
	switch (value)
	{
		...
		case GL_SAMPLER_2D_SHADOW: return FOSTER_UNIFORM_TYPE_SAMPLER2D;
	};
...
FosterTexture* FosterTextureCreate_OpenGL(int width, int height, FosterTextureFormat format)
{
	...
	switch (format)
	{
		...
		case FOSTER_TEXTURE_FORMAT_DEPTH24_STENCIL8:
			...
			break;
		case FOSTER_TEXTURE_FORMAT_R16:
			result.glInternalFormat = GL_R16;
			result.glFormat = GL_RED;
			result.glType =  GL_SHORT;
			break;
		default:
	...
	if (format == FOSTER_TEXTURE_FORMAT_DEPTH24_STENCIL8) 
	{
		fgl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE);
		fgl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_FUNC, GL_LESS);		
	}
...
void FosterTextureSetSubData_OpenGL(FosterTexture* texture, void* data, int offsetX, int offsetY, int width, int height)
{
	FosterTexture_OpenGL* tex = (FosterTexture_OpenGL*)texture;
	FosterBindTexture(0, tex->id);
	fgl.glTexSubImage2D(GL_TEXTURE_2D, 0, offsetX, offsetY, width, height, tex->glFormat, tex->glType, data);
}
...
FosterShader* FosterShaderCreate_OpenGL(FosterShaderData* data)
{
	...
	if (shader->uniformCount > 0)
	{
		...
		for (int i = 0; i < shader->uniformCount; i++)
		{
			...
			if (uniform->glType == GL_SAMPLER_2D || uniform->glType == GL_SAMPLER_2D_SHADOW)
			{
...
void FosterMeshSetIndexData_OpenGL(FosterMesh* mesh, void* data, int dataSize, int dataDestOffset)
{
	...
	if (it->indexBuffer == 0)
	{
		...
		fgl.stateElementBuffer = it->indexBuffer;
	}
	else if (fgl.stateElementBuffer != it->indexBuffer)
	{
		fgl.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, it->indexBuffer);
		fgl.stateElementBuffer = it->indexBuffer;
	}
...
bool FosterGetDevice_OpenGL(FosterRenderDevice* device)
{
	...
	device->textureSetSubData = FosterTextureSetSubData_OpenGL;
